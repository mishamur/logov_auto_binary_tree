...
namespace logov_auto_binary_tree
{
    public class BinaryTree<T> where T : IComparable<T>
    {
        ...
        private void _removeLevel(BinaryTree<T> node, int level)
        {
            if (level == Math.Floor((double)Height / 2))
            {
                remove(node.val);
            }
            else if (level > Math.Floor((double)Height / 2))
            {
                return;
            }
            else
            {
                if(node.right != null)
                    _removeLevel(node.right, level + 1);
                if (node.left != null)
                    _removeLevel(node.left, level + 1);
            }
        }
        ...
        public void _secondPrint(BinaryTree<T> node, int level)
        {
            if(node != null)
            {
                _secondPrint(node.right, level + 1);
                for(int i = 0; i <= level; i++)
                    Console.Write("  ");
                Console.WriteLine(node);
                _secondPrint(node.left, level + 1);
            }
        }
        ...
        private int _height(BinaryTree<T> root)
        {
            if (root == null)
                return 0;
